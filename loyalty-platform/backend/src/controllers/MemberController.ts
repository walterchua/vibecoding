...
export class MemberController {
  ...
  static async getPointsHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await MemberService.getPointsHistory(memberId, { page, limit, type });
      res.json(result);
    } catch (error) {
      next(error);
    }
  ...
  static async getTierProgress(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const memberId = req.memberId!;
      const progress = await MemberService.getTierProgress(memberId);
      res.json({
        currentTier: {
  ...
  static async getVouchers(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const vouchers = await MemberService.getMemberVouchers(memberId, status);
      res.json({ vouchers });
    } catch (error) {
      next(error);
    }
